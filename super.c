...
int aeon_get_nvmm_info(struct super_block *sb, struct aeon_sb_info *sbi);
...
static void aeon_fill_region_table(struct super_block *sb)
{
	...
	for (cpu_id = 0; cpu_id < sbi->cpus; cpu_id++) {
		...
		if (sbi->s_mount_opt & AEON_MOUNT_FORMAT) {
			...
			//aeon_dbg("cpuid %d head %lu blocknr %lu\n", cpu_id, inode_start, blocknr);
		} else {
...
struct file_system_type aeon_fs_type = {
	.owner		= THIS_MODULE,
	.name		= "aeon",
	.mount		= aeon_mount,
...
int aeon_get_nvmm_info(struct super_block *sb, struct aeon_sb_info *sbi)
{
	...
	bdev = blkdev_get_by_path("/dev/pmem1", mode, &aeon_fs_type);
	if (bdev)
		aeon_dbgv("1");
	if (bdev->bd_disk)
		aeon_dbgv("2");
	...
	dax_dev = fs_dax_get_by_host(sbi->s_bdev2->bd_disk->disk_name);
	if (!dax_dev) {
		aeon_err(sb, "Couldn't retrieve DAX device.\n");
		blkdev_put(bdev, mode);
		return -EINVAL;
	...
	aeon_dbgv("head addr 0x%llx\n", (u64)virt_addr);
	aeon_dbg("%s: dev %s, phys_addr 0x%llx, virt_addr 0x%lx, size %ld\n",
		__func__, sbi->s_bdev2->bd_disk->disk_name,
