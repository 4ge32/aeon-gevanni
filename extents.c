			  ...
			  int index)
{
	...
	numa_id = aeh->eh_numa_id[num_exblock];
						...
						struct aeon_extent_header *head)
{
	...
	numa_id = head->eh_numa_id[PI_MAX_EXTERNAL_EXTENT-1];
	next_addr <<= AEON_SHIFT;
	if (numa_id)
		aeh = (struct aeon_extent_header *)(next_addr + (u64)sbi->virt_addr2);
	else
					       ...
					       struct aeon_inode_info_header *sih)
{
	...
	num_exblock = le64_to_cpu(pi->i_exblocks) - 1;
	if (num_exblock < 0) {
	aeon_dbg("? %d\n",num_exblock);
		num_exblock = 0;
	}
	...
	if (!external_entries) {
		...
		aeh->eh_numa_id[next_num_exblock] = numa_id;
		pi->i_exblocks++;
		   ...
		   int numa_id)
{
	...
	struct aeon_extent_header *aeh = aeon_get_extent_header(pi);
	struct aeon_extent *ae;
	struct aeon_extent *prev = NULL;
	unsigned long next;
	int err = -ENOSPC;

	if (aeh && le32_to_cpu(aeh->eh_entries)) {
		ae = aeon_get_prev_extent(aeh);
			goto new_alloc;
		if (!ae)
			goto new_alloc;

		if (ae->ex_numa_id != numa_id)
			goto new_alloc;
...
new_alloc:
	...
	ae->ex_index = 1;
	if (prev)
		ae->ex_index = prev->ex_index + 1;
	ae->ex_length = cpu_to_le16(num_blocks);
	ae->ex_block = cpu_to_le64(blocknr);
	ae->ex_offset = cpu_to_le32(offset);
	if (numa_id >= 2 || numa_id < 0) {
		aeon_dbg("sinuno??? %d \n", numa_id);
	}
	ae->ex_numa_id = numa_id;
	aeh->eh_blocks += cpu_to_le16(num_blocks);
	...
	if (err) {
		write_unlock(&sih->i_meta_lock);
		if (prev)
		aeon_dbg("%d - %d : %d - %d\n", prev->ex_offset, prev->ex_length,
				ae->ex_offset, ae->ex_length);
		return err;
	}
			 ...
			 int depth)
{
	...
	for (i = 0; i < PI_MAX_EXTERNAL_EXTENT; i++) {
		...
		if (!blocknr)
			return 0;
		numa_id = aeh->eh_numa_id[i];
		err = aeon_insert_blocks_into_free_list(sb, blocknr, 1, 0, numa_id);
		if (err) {
			AEON_ERR(err);
			return err;
		}
...
next:
	...
	if (le16_to_cpu(aeh->eh_depth)) {
		...
		numa_id = aeh->eh_numa_id[PI_MAX_EXTERNAL_EXTENT-1];
		if (numa_id)
			next = (u64)sbi->virt_addr2 + (blocknr<<AEON_SHIFT);
		else
