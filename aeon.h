...
static inline int aeon_sb_issue_zeroout(struct super_block *sb, sector_t block,
		sector_t nr_blocks, int numa_id, gfp_t gfp_mask)
{
	if (numa_id)
		return blkdev_issue_zeroout(AEON_SB(sb)->s_bdev2, block << (sb->s_blocksize_bits -
					SECTOR_SHIFT),
				nr_blocks << (sb->s_blocksize_bits -
					SECTOR_SHIFT), gfp_mask, 0);
	else
		return blkdev_issue_zeroout(sb->s_bdev, block << (sb->s_blocksize_bits -
					SECTOR_SHIFT),
				nr_blocks << (sb->s_blocksize_bits -
					SECTOR_SHIFT), gfp_mask, 0);
}
