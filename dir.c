						       ...
						       int numa_id)
{
	...
	*d_blocknr = aeon_get_new_dentry_block(sb, &de_addr, numa_id);
	if (*d_blocknr == 0)
			    ...
			    struct aeon_inode_info_header *sih, u32 ino)
{
	...
	blocknr = aeon_get_new_dentry_block(sb, &de_addr, numa_id);
	if (blocknr == 0)
		return -ENOSPC;

	if (numa_id)
		de_addr_base = de_addr - (u64)sbi->virt_addr2;
	else
		de_addr_base = de_addr - (u64)sbi->virt_addr;

	if ((u64)pi > (u64)sbi->virt_addr2)
		pi_addr = (u64)pi - (u64)sbi->virt_addr2;
	else
					...
					int numa_id)
{
	...
	int numa_id_p = de_map->numa_id[global]; 

	if (numa_id_p) {
		de = (struct aeon_dentry *)((u64)sbi->virt_addr2 +
		...
		de_map->numa_id[de_map->num_latest_dentry] = numa_id;
	...
	if(!isInvalidSpace(de_info)) {
		if (de_map->num_internal_dentries == AEON_INTERNAL_ENTRY) {
			...
			*direntry = aeon_alloc_new_dentry_block(sb, &blocknr,
			...
			aeon_register_dentry_to_map(sb, de_map, blocknr, numa_id);
		} else {
			   ...
			   struct aeon_mdata *am, const char *name, int namelen)
{
	...
	if (am->pi_addr > (u64)sbi->virt_addr2)
		i_addr_offset = am->pi_addr - (u64)sbi->virt_addr2;
	else
		i_addr_offset = am->pi_addr - (u64)sbi->virt_addr;

	if ((u64)de > (u64)sbi->virt_addr2)
		d_addr_offset = (u64)de - (u64)sbi->virt_addr2;
	else
		d_addr_offset = (u64)de - (u64)sbi->virt_addr;

	if ((u64)am->pidir > (u64)sbi->virt_addr2)
		i_paddr_offset = (u64)am->pidir - (u64)sbi->virt_addr2;
	else
		i_paddr_offset = (u64)am->pidir - (u64)sbi->virt_addr;
		   ...
		   struct inode *inode, int update_times)
{
	...
	de->ino = pi->aeon_ino;
	if (sih->pi_addr > (u64)sbi->virt_addr2)
		de->d_inode_addr = cpu_to_le64(sih->pi_addr - (u64)sbi->virt_addr2);
	else
		de->d_inode_addr = cpu_to_le64(sih->pi_addr - (u64)sbi->virt_addr);
	if ((u64)pi > (u64)sbi->virt_addr2)
		de->d_pinode_addr = (u64)pi - (u64)sbi->virt_addr2;
	else
		de->d_pinode_addr = (u64)pi - (u64)sbi->virt_addr;
	aeon_update_dentry_csum(de);

	if ((u64)de > (u64)sbi->virt_addr2)
		pi->i_dentry_addr = cpu_to_le64((u64)de - (u64)sbi->virt_addr2);
	else
				   ...
				   struct aeon_inode_info_header *sih)
{
	...
	for (global = 1; global <= de_map->num_latest_dentry; global++) {
		...
		numa_id = de_map->numa_id[global];
		if (numa_id)
			virt_addr = (u64)sbi->virt_addr2;
		else
			virt_addr = (u64)sbi->virt_addr;
		//aeon_dbg("blocknr %lu\n", blocknr);
		for (internal = 0; internal < AEON_INTERNAL_ENTRY; internal++) {
			de = (struct aeon_dentry *)(virt_addr +
						    (blocknr << AEON_SHIFT) +
		...
		if (free) {
			err = aeon_insert_blocks_into_free_list(sb, blocknr,
						AEON_PAGES_FOR_DENTRY, 0, numa_id);
			if (err) {
				aeon_err(sb, "%s\n", __func__);
				return -EINVAL;
			}
			    ...
			    struct aeon_inode *pi, u64 *de_addr)
{
	...
	if (*de_addr > (u64)sbi->virt_addr2)
		*de_addr = (u64)sbi->virt_addr2 + addr;
	else
...
static int aeon_readdir(struct file *file, struct dir_context *ctx)
{
	...
	while (temp) {
		...
		if (!dir_emit(ctx, entry->name, entry->name_len,
			      ino, IF2DT(le16_to_cpu(entry->d_mode)))) {
			//aeon_dbg("%s: pos %lu\n", __func__, pos);
			return 0;
		}
