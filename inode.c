			   ...
			   struct aeon_dentry *de)
{
	...
	if (cpu_to_mem(cpu_id))
		*pi_addr = (u64)sbi->virt_addr2 + addr;
	else
				       ...
				       struct aeon_mdata *am)
{
	...
	if (am->de_addr > (u64)sbi->virt_addr2)
		d_addr_offset = am->de_addr - (u64)sbi->virt_addr2;
	else
		d_addr_offset = am->de_addr - (u64)sbi->virt_addr;
	if ((u64)pidir > (u64)sbi->virt_addr2)
		p_addr_offset = (u64)pidir - (u64)sbi->virt_addr2;
	else
			    ...
			    struct inode_map *inode_map, u32 ino)
{
	...
	//internal_ino = art->i_allocated-1;


//	aeon_dbg("INTERNAL %d\n", internal_ino);

	if (internal_ino < 0)
		aeon_dbg("inte %lu cpu %u\n", internal_ino, ino);
	...
	addr = virt_addr + (blocknr << AEON_SHIFT) +
		(internal_ino << AEON_I_SHIFT);

	//aeon_dbg("%d cpu %s ino %u addr 0x%llx\n", cpu_id, __func__, ino, addr);
	return addr;
}
...
err:
	aeon_err(sb, "can't alloc ino %u's inode address - addr %llu ret %lu\n", free_ino,
			*pi_addr, ret);
	return -ENOSPC;
}
...
int aeon_new_aeon_inode(struct super_block *sb, struct aeon_mdata *am)
{
	...
	am->ino = free_ino;
	am->pi_addr = pi_addr;
	am->p_numa_id = cpu_to_mem(cpu_id);
...
expand:
	...
	err = aeon_sb_issue_zeroout(sb, new_blocknr, allocated, numa_id, GFP_NOFS);
	if (err)
		aeon_err(sb, "%s: ERROR\n", __func__);
	mutex_unlock(&sih->truncate_mutex);
}
